...
def learn_from_batch(experiences):
    ...
    for experience in experiences:
        current_stack, action, reward, frame_stack, done = experience
        loss = agent.learn(current_stack, action, reward, frame_stack, done)
...
for e in range(episodes):
    ...
    while playing:
        ...
        print(f"Reward: {rewardTracker.sum_rewards():.3f}", end="\r", flush=True)
        ...
        #agent.soft_update()
        replay_buffer.add((current_stack, action, reward, frame_stack, dead), error)
        reward = 0
