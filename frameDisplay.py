...
class FrameDisplay:
    def __init__(self, frame_shape=(48, 64), scale=1, spacing=5, window_size=(640, 480), num_q_values=8):
    ...
    def display_frames(self, tensor, q_values):
        """
        Display frames from a PyTorch tensor and Q-values using OpenCV.

        Args:
        tensor (torch.Tensor): A tensor of shape (4, height, width).
        q_values (torch.Tensor): A tensor of Q-values.
        """
        if tensor.shape[1:] != self.frame_shape:
            raise ValueError(f"Each frame in the tensor must be of shape {self.frame_shape}")
        if len(q_values) != self.num_q_values:
            raise ValueError(f"Number of Q-values must be {self.num_q_values}")

        frames = self._prepare_frames(tensor)
    ...
    def _prepare_frames(self, tensor):
        tensor = tensor.float()
        tensor -= tensor.min()
        tensor /= tensor.max()
        tensor *= 255
    ...
    def _create_q_values_bar(self, q_values, width):
        ...
        """
        q_values = q_values.cpu().numpy()  # Convert to numpy
        ...
        for i, value in enumerate(q_values_normalized):
            ...
            # Label the bar
            cv2.putText(q_values_frame, str(i+1), (left + 5, self.bar_height - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.3, (0, 0, 0), 1, cv2.LINE_AA)
